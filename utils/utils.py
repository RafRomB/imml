...
def save_record(train_Xs, train_X, clusters, y, p, best_solution, y_true_total, elapsed_time, strat, random_state, errors_dict):
    ...
    if not all(missing_clusters_mask):
        ...
        summary_results = get_result_summary(y_true=y_excluding_missing, y_pred=clusters_excluding_missing,
    ...
    summary_results = get_result_summary(y_true=y, y_pred=clusters, p=p, best_solution=best_solution,
...
def get_result_summary(y_true, y_pred, p, best_solution, y_true_total, X, random_state):
    ...
    supervised_metrics = compute_supervised_metrics(y_true=y_true, y_pred=y_pred)
    if p > 0:
        ...
        performance_metrics = compute_supervised_metrics(y_true=best_solution_local, y_pred=y_pred)
        ...
        artificial_supervised_metrics = compute_supervised_metrics(y_true=y_true_total, y_pred=y_pred_artificial)
        ...
        artificial_performance_metrics = compute_supervised_metrics(y_true=best_solution, y_pred=y_pred_artificial)
    ...
    unsupervised_metrics = compute_unsupervised_metrics(X=X, y_pred=y_pred, random_state=random_state)
    summary_dict = {
...
def compute_supervised_metrics(y_true, y_pred):
    ...
    supervised_metrics = {
        **get_supervised_metrics(y_true=y_true, y_pred=y_pred),
...
def run_iteration(idx, results, Xs, y, n_clusters, algorithms, random_state, file_path, logs_file, error_file):
    ...
    try:
        ...
        if alg_name == "intNMF":
            try:
                model.run(trials=50, iterations=50, verbose=False)
                model.cluster_data()
            except ValueError as exception:
                errors_dict[f"{type(exception).__name__}: {exception}"] += 1
                exceptions = True
