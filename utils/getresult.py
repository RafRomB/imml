...
class GetResult:
    ...
    def run_iteration(idx, results, Xs, y, n_clusters, algorithms, random_state, subresults_path, logs_file, error_file):
        ...
        try:
            ...
            dict_results = GetResult.save_record(train_Xs=train_Xs, train_X=train_X, clusters=clusters, y=y_train, p=p,
    ...
    def save_record(train_Xs, train_X, clusters, y, p, best_solution, elapsed_time, strat, random_state, errors_dict):
        ...
        if not all(missing_clusters_mask):
            ...
            summary_results = GetResult.get_result_summary(y_true=y_excluding_missing, y_pred=clusters_excluding_missing,
        ...
        summary_results = GetResult.get_result_summary(y_true=y, y_pred=clusters, p=p, best_solution=best_solution,
    ...
    def get_result_summary(y_true, y_pred, p, best_solution, X, random_state):
        ...
        supervised_metrics = GetResult.compute_supervised_metrics(y_true=y_true, y_pred=y_pred)
        if p > 0:
            best_solution_local = best_solution.loc[y_pred.index]
            assert best_solution_local.index.equals(y_pred.index)
            performance_metrics = GetResult.compute_supervised_metrics(y_true=best_solution_local, y_pred=y_pred)
        ...
        unsupervised_metrics = GetResult.compute_unsupervised_metrics(X=X, y_pred=y_pred, random_state=random_state)
        summary_dict = {
    ...
    def compute_supervised_metrics(y_true, y_pred):
        ...
        supervised_metrics = {
            **GetResult.get_supervised_metrics(y_true=y_true, y_pred=y_pred),
