...
def test_convert_to_imvd(sample_data):
    ...
    transformed_Xs = DatasetUtils.convert_to_imvd(Xs_pandas, observed_view_indicator_pandas)
    assert len(transformed_Xs) == len(Xs_pandas)
    values_to_compare = [2,1]
    for i, X in enumerate(Xs_pandas):
        assert transformed_Xs[i].shape == X.shape
        assert np.isnan(transformed_Xs[i]).all(1).sum() == values_to_compare[i]
    transformed_Xs = DatasetUtils.convert_to_imvd(Xs_numpy, observed_view_indicator_numpy)
    for i, X in enumerate(Xs_numpy):
        assert transformed_Xs[i].shape == X.shape
        assert np.isnan(transformed_Xs[i]).all(1).sum() == values_to_compare[i]
...
def test_get_n_views(sample_data):
    ...
    n_views = DatasetUtils.get_n_views(Xs_pandas)
    assert n_views == len(Xs_pandas)
    n_views = DatasetUtils.get_n_views(Xs_numpy)
    assert n_views == len(Xs_numpy)

def test_get_n_samples_by_view(sample_data):
    Xs_pandas, observed_view_indicator_pandas, Xs_numpy, observed_view_indicator_numpy = sample_data
    n_samples_by_view = DatasetUtils.get_n_samples_by_view(Xs_pandas)
    values_to_compare = [3,4]
    assert all(n_samples_by_view == values_to_compare)
    n_samples_by_view = DatasetUtils.get_n_samples_by_view(Xs_numpy)
    assert all(n_samples_by_view == values_to_compare)

def test_get_complete_sample_names(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    complete_samples = DatasetUtils.get_complete_sample_names(Xs_pandas)
    assert len(complete_samples) == 2
    values_to_compare = pd.Index(["a","d"])
    assert complete_samples.equals(values_to_compare)
    complete_samples = DatasetUtils.get_complete_sample_names(Xs_numpy)
    assert len(complete_samples) == 2
...
def test_get_incomplete_sample_names(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    incomplete_samples = DatasetUtils.get_incomplete_sample_names(Xs_pandas)
    assert len(incomplete_samples) == 3
    values_to_compare = pd.Index(["b","c","e"])
    assert incomplete_samples.equals(values_to_compare)
    incomplete_samples = DatasetUtils.get_incomplete_sample_names(Xs_numpy)
    assert len(incomplete_samples) == 3
...
def test_get_samples_by_view(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    samples_by_view = DatasetUtils.get_samples_by_view(Xs_pandas, return_as_list=True)
    ...
    samples_by_view = DatasetUtils.get_samples_by_view(Xs_pandas, return_as_list=False)
    assert len(samples_by_view) == len(Xs_pandas)
    for i in range(len(samples_by_view)):
        assert samples_by_view[i].equals(pd.Index(values_to_compare[i]))
    samples_by_view = DatasetUtils.get_samples_by_view(Xs_numpy, return_as_list=True)
    values_to_compare = [[0,1,3], [0,2,3, 4]]
    assert len(samples_by_view) == len(Xs_numpy)
    for i in range(len(samples_by_view)):
        assert samples_by_view[i].equals(pd.Index(values_to_compare[i]))

def test_get_missing_samples_by_view(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    missing_samples_by_view = DatasetUtils.get_missing_samples_by_view(Xs_pandas, return_as_list=True)
    assert len(missing_samples_by_view) == len(Xs_pandas)
    ...
    missing_samples_by_view = DatasetUtils.get_missing_samples_by_view(Xs_pandas, return_as_list=False)
    assert len(missing_samples_by_view) == len(Xs_pandas)
    values_to_compare = [["c","e"], ["b"]]
    for i in range(len(missing_samples_by_view)):
        assert pd.Index(missing_samples_by_view[i]).equals(pd.Index(values_to_compare[i]))
    missing_samples_by_view = DatasetUtils.get_missing_samples_by_view(Xs_numpy, return_as_list=True)
    assert len(missing_samples_by_view) == len(Xs_numpy)
...
def test_get_n_complete_samples(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    n_complete_samples = DatasetUtils.get_n_complete_samples(Xs_pandas)
    assert n_complete_samples == 2
    n_complete_samples = DatasetUtils.get_n_complete_samples(Xs_numpy)
    assert n_complete_samples == 2

def test_get_n_incomplete_samples(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    n_incomplete_samples = DatasetUtils.get_n_incomplete_samples(Xs_pandas)
    assert n_incomplete_samples == 3
    n_incomplete_samples = DatasetUtils.get_n_incomplete_samples(Xs_numpy)
    assert n_incomplete_samples == 3

def test_get_percentage_complete_samples(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    percentage_complete = DatasetUtils.get_percentage_complete_samples(Xs_pandas)
    assert percentage_complete == (2 / 5) * 100
    percentage_complete = DatasetUtils.get_percentage_complete_samples(Xs_numpy)
    assert percentage_complete == (2 / 5) * 100

def test_get_percentage_incomplete_samples(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    percentage_incomplete = DatasetUtils.get_percentage_incomplete_samples(Xs_pandas)
    assert percentage_incomplete == (3 / 5) * 100
    percentage_incomplete = DatasetUtils.get_percentage_incomplete_samples(Xs_numpy)
    assert percentage_incomplete == (3 / 5) * 100

def test_remove_missing_sample_from_view(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    cleaned_Xs = DatasetUtils.remove_missing_sample_from_view(Xs_pandas)
    values_to_compare = [3, 4]
    for to_compare, X in zip(values_to_compare, cleaned_Xs):
        assert len(X) == to_compare
    cleaned_Xs = DatasetUtils.remove_missing_sample_from_view(Xs_numpy)
    for to_compare, X in zip(values_to_compare, cleaned_Xs):
        assert len(X) == to_compare
...
def test_convert_mvd_from_list_to_dict(sample_data):
    Xs_pandas, _, Xs_numpy, _ = sample_data
    Xs_dict = DatasetUtils.convert_mvd_from_list_to_dict(Xs_pandas)
    assert isinstance(Xs_dict, dict)
    assert len(Xs_dict) == len(Xs_numpy)
    assert all(isinstance(k, int) for k in Xs_dict.keys())
    Xs_dict = DatasetUtils.convert_mvd_from_list_to_dict(Xs_numpy)
    assert isinstance(Xs_dict, dict)
    assert len(Xs_dict) == len(Xs_numpy)
    assert all(isinstance(k, int) for k in Xs_dict.keys())
...
def test_convert_mvd_from_dict_to_list(sample_data):
    ...
    assert len(Xs_list) == len(Xs_pandas)
    for X, X_converted in zip(Xs_pandas, Xs_list):
        assert X.equals(X_converted)
    Xs_dict = DatasetUtils.convert_mvd_from_list_to_dict(Xs_numpy)
    Xs_list = DatasetUtils.convert_mvd_from_dict_to_list(Xs_dict)
    assert len(Xs_list) == len(Xs_numpy)
    for X, X_converted in zip(Xs_numpy, Xs_list):
