...
def test_MOFA_default(sample_data):
    transformer = MOFA()
...
def test_MOFA_init(sample_data):
    n_components = 5
...
def test_MOFA_transform(sample_data):
    n_components = 5
    transformer = MOFA(n_components=n_components, random_state=42)
    for Xs in sample_data:
        n_samples = len(Xs[0])
...
def test_MOFA_set_output(sample_data):
...
def test_MOFA_invalid_params(sample_data):
